...
now uses Pathom for server-side mutation and query handling.  See `routing/server_components/pathom_wrappers.clj` and
`routing/model/user.clj` for the macro definitions and examples of how to define
...
See the server section below for working on the full-stack routing itself.

(ns routing.model.user
  (:require
    [com.wsscode.pathom.connect :as pc]
    [routing.server-components.pathom-wrappers :refer [defmutation defresolver]]
...
(defresolver task-set-resolver
  [env {:task-set/keys [id]}]
  {::pc/input  #{:task-set/id}
   ::pc/output [:task-set/title]}
  (Thread/sleep 500)
  {:task-set/id    id
   :task-set/title (str "Task set " id)})
